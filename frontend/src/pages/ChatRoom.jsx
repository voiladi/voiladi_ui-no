...
const ReactionBubble = ({ m, mine, otherName }) => {
  ...
  return (
    <div
      ...
      data-testid="chat-reaction-message"
    >
      {r.type === "photo" ? (
        ...
        <div className="vo-soft max-w-[82%] rounded-[20px] px-4 py-3">
          <div className="mb-1 text-[12px] font-medium text-mute">
            {r.question}
          </div>
          <div className="text-[15px] leading-snug text-ink">{r.answer}</div>
        </div>
      )}
      <ReactionPill className="mt-1.5">
        {reactionLabel(r, { mine })}
      </ReactionPill>
    </div>
  );
};
...
export default function ChatRoom() {
  ...
  const suggestions = useMemo(
    () => shuffle(meta.icebreakers || []).slice(0, 3),
    [meta.icebreakers],
  );
  const drawerBreakers = useMemo(
    () => shuffle(meta.icebreakers || []).slice(0, 10),
    [meta.icebreakers, icebreakers],
  ); // eslint-disable-line react-hooks/exhaustive-deps

  const isDm = match?.kind === "dm";
  const isRequestForMe = !!match?.is_request;
  const requestSentByMe =
    isDm && match?.status === "request" && match?.requested_by === user?.id;
  ...
  const mergeMessages = useCallback((incoming) => {
    setMessages((prev) => {
      ...
      const byClient = new Map(
        prev.filter((m) => m.client_id).map((m) => [m.client_id, m]),
      );
      const next = [...prev];
      incoming.forEach((m) => {
    ...
    () =>
      subscribe((ev) => {
        ...
        if (ev.type === "message") {
          ...
          if (ev.accepted)
            setMatch((m) =>
              m ? { ...m, status: "active", is_request: false } : m,
            );
  ...
  const sendMessage = async (override) => {
    ...
    setSending(true);
    try {
      const { data } = await api.post(`/matches/${matchId}/messages`, {
        text: body,
        client_id,
      });
      mergeMessages([data]);
      if (isRequestForMe)
        setMatch((m) => ({ ...m, status: "active", is_request: false }));
      qc.invalidateQueries({ queryKey: ["matches"] });
    } catch (e) {
  ...
  const sendMedia = async ({ viewOnce }) => {
    ...
    const temp = {
      ...
      kind,
      text: "",
      media: { kind, view_once: viewOnce, status: "ready" },
      created_at: new Date().toISOString(),
      read_at: null,
      pending: true,
      progress: 0,
    ...
    setMessages((prev) => [...prev, temp]);
    try {
      const data = await uploadChatMedia({
        file,
        matchId,
        viewOnce,
        clientId: client_id,
        duration,
        onProgress: (p) => patchMessage(temp.id, { progress: p }),
      });
      mergeMessages([
        {
          ...data,
          localUrl:
            kind === "video" && data.media?.status === "processing"
              ? localUrl
              : undefined,
        },
      ]);
  ...
  const openViewOnce = async (m) => {
    ...
    try {
      const { data } = await api.post(
        `/matches/${matchId}/messages/${m.id}/open`,
      );
      patchMessage(m.id, {
        media: {
          ...m.media,
          opened_at: m.media.opened_at || new Date().toISOString(),
        },
      });
      setViewer({ kind: data.kind, url: mediaUrl(data.url), viewOnce: true });
    } catch (e) {
      if (e?.response?.status === 410)
        patchMessage(m.id, { media: { ...m.media, status: "expired" } });
  ...
  const rendered = useMemo(() => {
    ...
    messages.forEach((m, idx) => {
      ...
      const endOfGroup =
        !nextM ||
        nextM.sender_id !== m.sender_id ||
        new Date(nextM.created_at) - new Date(m.created_at) > 5 * 60 * 1000;
      const startOfGroup =
        !prevM ||
        prevM.sender_id !== m.sender_id ||
        new Date(m.created_at) - new Date(prevM.created_at) > 5 * 60 * 1000;
  ...
  const status = typing
    ...
    : match?.online
      ? "Active now"
      : activeLabel(other?.last_active) || "";

  const intro = () => {
    if (!other) return null;
    const ago =
      timeAgo(match.created_at) === "now"
        ? "just now"
        : `${timeAgo(match.created_at)} ago`;
    if (isRequestForMe)
      return (
        <>
          <span className="font-semibold text-ink">{other.name}</span> wants to
          send you a message · {ago}
        </>
      );
    if (isDm && requestSentByMe)
      return (
        <>
          You sent <span className="font-semibold text-ink">{other.name}</span>{" "}
          a message request · {ago}. They'll see it under Requests.
        </>
      );
    if (isDm)
      return (
        <>
          You and <span className="font-semibold text-ink">{other.name}</span>{" "}
          started talking {ago}
        </>
      );
    return (
      <>
        You matched with{" "}
        <span className="font-semibold text-ink">{other.name}</span> {ago}
        {other.shared_interests?.length
          ? ` · you both like ${other.shared_interests[0]}`
          : ""}
      </>
    );
  };
  ...
  return (
    <div
      className="vo-neu-page relative flex h-full flex-col"
    ...
    >
      ...
      <header className="vo-chat-header z-10 flex h-[72px] shrink-0 items-center gap-2 px-3">
        <button
          type="button"
          className="-ml-1 flex h-11 w-10 shrink-0 items-center justify-center text-ink active:opacity-60 focus-visible:outline-none"
        ...
        {other ? (
          <button
            type="button"
            className="flex min-w-0 flex-1 items-center gap-3 py-1 text-left active:opacity-70 focus-visible:outline-none"
          ...
          >
            <span className="relative shrink-0">
              <span className="vo-avatar-ring block">
                <UserPhoto
                  ...
                  name={other.name}
                  size="xs"
                  className="h-[46px] w-[46px] rounded-full text-[17px]"
                />
              </span>
              {match.online && (
                <span
                  className="vo-dot-online bottom-0.5 right-0.5 h-3.5 w-3.5 border-[2.5px]"
            ...
            <span className="min-w-0">
              <span className="flex items-center gap-1.5 text-[20px] font-bold leading-[1.15] tracking-[-0.02em] text-ink">
                ...
                {other.verified && (
                  <VerifiedBadge size={17} testId="chat-header-verified" />
                )}
              </span>
              {status && (
                <span
                  className="block truncate whitespace-nowrap text-[14px] leading-[1.2] tracking-[-0.01em] text-mute"
        ...
        <button
          type="button"
          className="vo-glass-icon h-[44px] w-[44px]"
          onClick={() => notice("Voice calls are coming soon")}
        ...
        <button
          type="button"
          className="vo-glass-icon h-[44px] w-[44px]"
          onClick={() => notice("Video calls are coming soon")}
          aria-label="Video call"
          data-testid="chat-video-call-button"
        >
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent
            align="end"
            className="w-56 rounded-[18px] border-0 bg-canvas p-1.5 shadow-modal"
          >
            <DropdownMenuItem
              className="rounded-[12px] py-2.5 text-[15px]"
              onClick={() => setSheet(true)}
            ...
            {isDm ? (
              <DropdownMenuItem
                ...
                data-testid="chat-menu-delete-chat"
              >
            ...
            <DropdownMenuItem
              className="rounded-[12px] py-2.5 text-[15px] text-red focus:text-red"
      ...
      {!connected && !loading && (
        <div
          className="mx-auto mb-1 inline-flex h-[26px] items-center rounded-full bg-[#262626] px-3 text-[12px] font-medium text-white"
      ...
      <div
        ...
        className={`vo-scroll px-3.5 pt-2 ${composerHidden || !user?.verified || loading ? "pb-3" : "pb-[84px]"}`}
        data-testid="chat-messages"
      >
        {loading ? (
          <div
            className="flex h-full min-h-[40vh] items-center justify-center text-ink"
          ...
          <>
              ...
              >
                ...
                >
                  <UserPhoto
                    ...
                    name={user?.name}
                    size="xs"
                    className="h-16 w-16 rounded-full border-[3px] border-canvas text-lg"
                  />
                  <UserPhoto
                    src={other.photos?.[0]}
                    name={other.name}
                    size="xs"
                    className="-ml-4 h-16 w-16 rounded-full border-[3px] border-canvas text-lg"
                  />
                </button>
                <p
                  className="mt-3 max-w-[300px] text-[13px] leading-[17px] text-mute"
                  data-testid="chat-intro-text"
                >
                  {intro()}
                </p>
                {noTextYet && !composerHidden && !isDm && (
                  <div className="mt-5 w-full">
                    <div className="mb-2.5 text-[12px] font-semibold text-mute">
                      Vibe check
                    </div>
                    <div className="flex flex-col gap-2">
                      {suggestions.map((s) => (
                        <button
                          key={s}
                          type="button"
                          className="vo-soft h-auto justify-start whitespace-normal rounded-[18px] px-4 py-2.5 text-left text-[14px] text-ink active:opacity-80"
            ...
            <AnimatePresence initial={false}>
              {rendered.map((r) => {
                ...
                return r.type === "day" ? (
                  ...
                  >
                    <span
                      className="vo-day-pill inline-flex h-[30px] items-center rounded-full px-4 text-[14px] font-medium text-mute"
                      data-testid="chat-day-label"
                    >
                      {r.label}
                    </span>
                  </div>
                  ...
                  >
                      ...
                      <span className="mr-2.5 w-[38px] shrink-0">
                        {r.startOfGroup && (
                          <UserPhoto
                            ...
                            name={other?.name}
                            size="xs"
                            className="h-[38px] w-[38px] rounded-full text-[14px]"
                    ...
                    <div
                      className={`flex max-w-[76%] flex-col ${mine ? "items-end" : "items-start"}`}
                    >
                      {r.m.kind === "reaction" ? (
                        <ReactionBubble
                          m={r.m}
                        ...
                        <LongPress
                          enabled={mine && !r.m.pending}
                          onFire={() => setActionMsg(r.m)}
                        >
                          {r.m.media?.view_once ? (
                            <ViewOnceBubble
                              m={r.m}
                            ...
                            <MediaBubble
                              ...
                              mine={mine}
                              onOpen={(item) =>
                                setViewer({ ...item, name: other?.name })
                              }
                            />
                          )}
                        </LongPress>
                      ...
                      <div
                        className={`mt-1.5 flex items-center gap-1.5 px-1 text-[13.5px] leading-none text-mute ${mine ? "justify-end" : ""}`}
                      ...
                      >
                        ...
                        {mine &&
                          ...
                          (r.m.read_at ? (
                            <CheckCheck
                              className="vo-tick h-4 w-4"
                              strokeWidth={2.4}
                              data-testid="chat-read-ticks"
                            />
                          ) : (
                            <Check
                              className="h-4 w-4"
                              strokeWidth={2.4}
                              data-testid="chat-sent-tick"
                            />
                          ))}
                      </div>
                      {mine && r.m.id === lastMine?.id && r.m.read_at && (
                        <span
                          className="mt-1 px-1 text-[13.5px] leading-none text-mute"
            ...
            {typing && (
              <motion.div
                ...
                className="mb-2 flex items-start justify-start"
      ...
      {loading ? null : isRequestForMe ? (
        ...
        >
          <div className="vo-soft rounded-[24px] p-4">
            <p className="text-[15px] font-semibold leading-[19px] tracking-[-0.01em] text-ink">
              {other?.name} wants to send you a message
            </p>
            <p className="mt-1 text-[13px] leading-[17px] text-mute">
              Accept to reply. They won't know you've seen this until you do.
              Not a match - deleting just removes the request.
            </p>
            <div className="mt-3.5 flex gap-2.5">
              <button
                ...
                onClick={() => setConfirm("decline")}
                disabled={busy}
                className="vo-soft-btn h-[46px] flex-1 text-[15px]"
              ...
              <button
                ...
                disabled={busy}
                aria-busy={busy}
                className="inline-flex h-[46px] flex-1 items-center justify-center rounded-full bg-ink text-[15px] font-semibold text-onink active:scale-[0.98] disabled:opacity-60"
                style={{
            ...
            <button
              type="button"
              onClick={() => setConfirm("block")}
              className="mt-3 block w-full text-center text-[13px] font-medium text-mute active:opacity-60"
          ...
          <div className="vo-soft flex items-center gap-3 rounded-[24px] p-3.5">
            <span className="min-w-0 flex-1">
              ...
              <span className="block text-[13px] text-mute">
                A quick selfie gets you the black tick.
              </span>
            </span>
            <button
              type="button"
              onClick={() => navigate("/settings/verification")}
              className="h-[40px] shrink-0 rounded-full bg-ink px-5 text-[15px] font-semibold text-onink active:scale-95"
              style={{
                transitionProperty: "transform",
                transitionDuration: "120ms",
              }}
              data-testid="chat-verify-button"
            >
              Verify
            </button>
          </div>
        ...
        <>
          ...
          >
            ...
            <div
              className="vo-chat-composer pointer-events-auto flex items-end gap-2 p-[6px]"
            ...
            >
              ...
              <motion.div
                ...
                animate={
                  composerExpanded
                    ? { width: 0, opacity: 0, scale: 0.6, marginRight: -8 }
                    : { width: 40, opacity: 1, scale: 1, marginRight: 0 }
                }
              ...
              >
                <button
                  type="button"
                  className="vo-plus-btn flex h-[40px] w-[40px] shrink-0 items-center justify-center rounded-full"
                  onClick={() => fileRef.current?.click()}
                  aria-label="Add a photo or video"
                  tabIndex={composerExpanded ? -1 : 0}
              ...
              <motion.div
                layout
                transition={tween(0.22)}
                className="vo-chat-input flex min-h-[40px] flex-1 items-end rounded-full px-3.5"
              >
                <textarea
                  ref={inputRef}
                  rows={1}
              ...
              <div className="flex h-[40px] shrink-0 items-center">
                <motion.div
                  ...
                  animate={
                    composerExpanded
                      ? { width: 0, opacity: 0, scale: 0.7 }
                      : { width: 102, opacity: 1, scale: 1 }
                  }
                  transition={tween(0.22)}
                ...
                >
                  <button
                    type="button"
                    className="flex h-[40px] w-[34px] items-center justify-center text-ink active:opacity-60 focus-visible:outline-none"
                    onClick={() => setIcebreakers(true)}
                    aria-label="Vibe check prompts"
                    tabIndex={composerExpanded ? -1 : 0}
                    data-testid="chat-icebreakers-open-button"
                  >
                  ...
                  <button
                    type="button"
                    className="flex h-[40px] w-[34px] items-center justify-center text-ink active:opacity-60 focus-visible:outline-none"
                    onClick={() => notice("Voice messages are coming soon")}
      ...
      <Drawer open={!!actionMsg} onOpenChange={(o) => !o && setActionMsg(null)}>
        <DrawerContent
          className="mx-auto max-w-[430px] rounded-t-[28px] border-0 bg-canvas"
          data-testid="message-actions-sheet"
        >
          <DrawerTitle className="sr-only">Message options</DrawerTitle>
          <DrawerDescription className="sr-only">
            Unsend or copy this message
          </DrawerDescription>
          <div className="px-5 pb-8 pt-3">
            {actionMsg && (
              <p
                className="vo-soft-sunken mb-4 max-h-[88px] overflow-hidden text-ellipsis whitespace-pre-wrap break-words rounded-[18px] px-4 py-3 text-[14px] leading-[19px] text-ink"
                data-testid="message-actions-preview"
              >
                {actionMsg.media
                  ? `${actionMsg.media.kind === "video" ? "Video" : "Photo"}${actionMsg.media.view_once ? " (view once)" : ""}`
                  : actionMsg.text}
              </p>
            )}
            <div className="flex flex-col gap-2.5">
              {actionMsg && !actionMsg.media && (
                <button
                  type="button"
                  className="vo-soft-row min-h-[54px] justify-start text-[16px] font-semibold text-ink"
                  onClick={copyText}
              ...
              <button
                type="button"
                className="vo-soft-row min-h-[54px] justify-start text-[16px] font-semibold text-red"
                onClick={unsend}
                data-testid="message-unsend-button"
              >
                <Undo2 className="h-5 w-5" strokeWidth={2} /> Unsend
              </button>
            </div>
            <p className="mt-3 text-center text-[12.5px] text-mute">
              Unsending removes the message for both of you.
            </p>
          </div>
        </DrawerContent>
      </Drawer>
      ...
      <Drawer open={icebreakers} onOpenChange={setIcebreakers}>
        <DrawerContent
          className="mx-auto max-h-[80dvh] max-w-[430px] rounded-t-[28px] border-0 bg-canvas"
          data-testid="icebreakers-drawer"
        >
          <DrawerTitle className="mt-3 px-5 text-[18px] font-bold text-ink">
            Vibe check
          </DrawerTitle>
          <DrawerDescription className="mb-3 mt-1 px-5 text-[13px] text-mute">
            Tap one to drop it into the chat. Edit it or send as is.
          </DrawerDescription>
      ...
      <MediaPreviewSheet
        ...
        tooLong={
          !!pendingDuration && pendingDuration > MEDIA_MAX_VIDEO_SECONDS + 1
        }
      />
      ...
      <ConfirmDialog
        ...
        description={
          isRequestForMe
            ? `${other?.name} won't be told. You can still find them in Explore.`
            : "This removes the conversation for both of you."
        }
        confirmText="Delete"
        danger
        loading={busy}
